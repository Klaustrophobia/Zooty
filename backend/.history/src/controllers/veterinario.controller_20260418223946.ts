...
export class VeterinarioController {
    ...
    try {
      ...
      if (!nombre_negocio || !correo || !telefono || !direccion) {
        return NextResponse.json(
          {
            error:
              "nombre_negocio, correo, telefono y direccion son requeridos",
          },
          { status: 400 },
        );
      }
  ...
  async getVeterinarioById(request: NextRequest, id: string) {
    try {
      ...
      console.error(
        "Error en VeterinarioController.getVeterinarioById:",
        error,
      );
      return NextResponse.json(
        { error: "Error interno del servidor" },
  ...
  async updateVeterinario(request: NextRequest, id: string) {
    try {
      ...
      const veterinarioExistente =
        await this.veterinarioService.getVeterinarioById(id);
      ...
      const veterinarioActualizado =
        await this.veterinarioService.updateVeterinario(id, {
          nombre_negocio,
          correo,
          telefono,
          direccion,
  ...
  async getVeterinariosCercanos(request: NextRequest) {
    try {
      ...
      const veterinarios =
        await this.veterinarioService.getVeterinariosCercanos(
          latitud,
          longitud,
          radioKm,
        );
      ...
      console.error(
        "Error en VeterinarioController.getVeterinariosCercanos:",
        error,
      );
      return NextResponse.json(
        { error: "Error interno del servidor" },
